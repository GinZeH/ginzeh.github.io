...
// 广告配置1 - 纯图片广告（无边距）
export const adConfig1: AdConfig = adSettings.enable ? {
	image: {
		src: adSettings.adImage ?? "assets/images/cover.avif",
		alt: "广告横幅",
		link: adSettings.adLink ?? "#",
		external: adSettings.adLinkExternal ?? true,
	},

	// 是否允许关闭广告
	closable: adSettings.adClosable ?? true,

	// 显示次数限制，-1为无限制
	displayCount: adSettings.adDisplayCount ?? -1,
